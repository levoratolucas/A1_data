...
for grupo in grupos:
    ...
    df['nps'] = df['nota'].apply(lambda x: 'detrator' if x < 7 else ('neutro' if x < 9 else 'promotor'))
    # _____________________________________________________________________________________________________
    ...
    # _______________________________________ tarefa 2 e 3 
    
    if grupo == 'Grupo 9_10':
        dfBrasil = df[(df['mercado'] == 'BRASIL') & ((df['Grupo de Produto'] == 'Grupo 9') | (df['Grupo de Produto'] == 'Grupo 10'))]
    else:
    ...
    contagem_classes.to_csv('{}_nps_brasil.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
    ...
    # Salvar o DataFrame em um arquivo CSV com delimitador ';' e codificação UTF-8
    contagem_classes.to_csv('{}_nps_regiao.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
    ...
    tabela_final.to_csv("{}_safra.csv".format(grupo), index=True)
    ...
    correlacao_nota_df.to_csv(f'{grupo}_spearman.csv', sep=';', encoding='utf-8', index=False)
    ...
    periodos_unicos = df['Periodo de Pesquisa'].unique()
    ...
    for periodo in periodos_unicos:
        ...
        # Detratores
        X_detrator = df_periodo[colunas_grupo]
        y_detrator = df_periodo['nps'].map({'detrator': 1, 'neutro': 0, 'promotor': 0})
        ...
        model_detrator_XGB = XGBClassifier(n_estimators=60, verbosity=3, random_state=42)
        model_detrator_XGB.fit(X_train, y_train)
        modelos_detrator_XGB[periodo] = model_detrator_XGB

        # Modelo Random Forest para detratores
        model_detrator_Random = RandomForestClassifier(n_estimators=60, random_state=42)
        model_detrator_Random.fit(X_train, y_train)
        modelos_detrator_Random[periodo] = model_detrator_Random

        # Neutros
        X_neutro = df_periodo[colunas_grupo]
        y_neutro = df_periodo['nps'].map({'detrator': 0, 'neutro': 1, 'promotor': 0})
        ...
        model_neutro_XGB = XGBClassifier(n_estimators=60, verbosity=3, random_state=42)
        model_neutro_XGB.fit(X_train, y_train)
        modelos_neutro_XGB[periodo] = model_neutro_XGB

        # Modelo Random Forest para neutros
        model_neutro_Random = RandomForestClassifier(n_estimators=60, random_state=42)
        model_neutro_Random.fit(X_train, y_train)
        modelos_neutro_Random[periodo] = model_neutro_Random

        # Promotores
        X_promotor = df_periodo[colunas_grupo]
        y_promotor = df_periodo['nps'].map({'detrator': 1, 'neutro': 1, 'promotor': 0})
        ...
        model_promotor_XGB = XGBClassifier(n_estimators=60, verbosity=3, random_state=42)
        model_promotor_XGB.fit(X_train, y_train)
        modelos_promotor_XGB[periodo] = model_promotor_XGB

        # Modelo Random Forest para promotores
        model_promotor_Random = RandomForestClassifier(n_estimators=60, random_state=42)
        model_promotor_Random.fit(X_train, y_train)
        modelos_promotor_Random[periodo] = model_promotor_Random
    ...
    # Loop pelas regiões
    for periodo in periodos_unicos:
        print(f"\nMétricas para a região: {periodo}")
        
        # Detratores
        model_detrator_XGB = modelos_detrator_XGB[periodo]
        model_detrator_Random = modelos_detrator_Random[periodo]
        print("\nDetratores (XGBoost):")
        # calcular_metricas_e_roc(model_detrator_XGB, X_test, y_test, f"Detratores - {periodo} (XGBoost)")
        print("\nDetratores (Random Forest):")
        # calcular_metricas_e_roc(model_detrator_Random, X_test, y_test, f"Detratores - {periodo} (Random Forest)")
        
        # Neutros
        model_neutro_XGB = modelos_neutro_XGB[periodo]
        model_neutro_Random = modelos_neutro_Random[periodo]
        print("\nNeutros (XGBoost):")
        # calcular_metricas_e_roc(model_neutro_XGB, X_test, y_test, f"Neutros - {periodo} (XGBoost)")
        print("\nNeutros (Random Forest):")
        # calcular_metricas_e_roc(model_neutro_Random, X_test, y_test, f"Neutros - {periodo} (Random Forest)")
        
        # Promotores
        model_promotor_XGB = modelos_promotor_XGB[periodo]
        model_promotor_Random = modelos_promotor_Random[periodo]
        print("\nPromotores (XGBoost):")
        # calcular_metricas_e_roc(model_promotor_XGB, X_test, y_test, f"Promotores - {periodo} (XGBoost)")
        print("\nPromotores (Random Forest):")
        # calcular_metricas_e_roc(model_promotor_Random, X_test, y_test, f"Promotores - {periodo} (Random Forest)")
...
# Loop por todas as regiões e todos os modelos
    ...
    df_top_10_xgb.to_csv('{}_XGBOOST.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
    df_top_10_random.to_csv('{}_RANDOM.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
    top_10_xgbperiodo = []
    top_10_randomperiodo = []
    
    for periodo in periodos_unicos:
    # Para XGBoost
        if periodo in modelos_detrator_XGB:
            top_10_xgbperiodo.append(extrair_top_10_importancias(modelos_detrator_XGB[periodo], colunas_grupo, f'XGBoost_Detrator_{periodo}'))
            top_10_xgbperiodo.append(extrair_top_10_importancias(modelos_neutro_XGB[periodo], colunas_grupo, f'XGBoost_Neutro_{periodo}'))
            top_10_xgbperiodo.append(extrair_top_10_importancias(modelos_promotor_XGB[periodo], colunas_grupo, f'XGBoost_Promotor_{periodo}'))
    
    # Para Random Forest
        if periodo in modelos_detrator_Random:
            top_10_randomperiodo.append(extrair_top_10_importancias(modelos_detrator_Random[periodo], colunas_grupo, f'Random_Detrator_{periodo}'))
            top_10_randomperiodo.append(extrair_top_10_importancias(modelos_neutro_Random[periodo], colunas_grupo, f'Random_Neutro_{periodo}'))
            top_10_randomperiodo.append(extrair_top_10_importancias(modelos_promotor_Random[periodo], colunas_grupo, f'Random_Promotor_{periodo}'))
   

    print("CSV para XGBoost e Random Forest gerados com as top 10 variáveis.")
    df_top_10_xgbperiodo = pd.concat(top_10_xgbperiodo)
    df_top_10_randomperiodo = pd.concat(top_10_randomperiodo)

    # Salvando os resultados em CSV
    df_top_10_xgbperiodo.to_csv('{}_XGBOOSTPERIODO.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
    df_top_10_randomperiodo.to_csv('{}_RANDOMPERIODO.csv'.format(grupo), sep=';', encoding='utf-8', index=False)
